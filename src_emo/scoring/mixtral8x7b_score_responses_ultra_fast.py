#!/usr/bin/env python3
"""
Mixtral8x7B-Instruct scoring script for conversational responses.
Optimized for ultra-fast scoring without memory constraints.
"""
...
def create_mixtral_prompt_ultra_fast(context, response):
...
def get_mixtral_scores_ultra_fast(prompt_string, model, tokenizer):
...
def main():
    ...
    parser.add_argument('--input', type=str, default='mixtral8x7b_prompts.jsonl', help='Input file')
    parser.add_argument('--output', type=str, default='mixtral8x7b_scored_ultra_fast.jsonl', help='Output file')
    parser.add_argument('--max', type=int, default=None, help='Max number of prompts to process (for testing)')
    args = parser.parse_args()

    print("Loading Mixtral8x7B-Instruct model and tokenizer with 4-bit quantization...")
    ...
    # Custom device map to handle large model
    device_map = {
    ...
    with open(args.input) as fin, open(args.output, "w") as fout:
        for i, line in enumerate(fin):
            ...
            prompt = create_mixtral_prompt_ultra_fast(context, response)
            scores, full_output = get_mixtral_scores_ultra_fast(prompt, model, tokenizer)
            if scores:
                successful_parses += 1
                overall = sum(scores.values()) / 4.0
                scores["overall_score"] = round(overall, 4)
                item["mixtral8x7b_scores"] = scores
                fout.write(json.dumps(item, ensure_ascii=False) + "\n")
                all_scores.append(scores)
            else:
                print(f"\n[PARSE FAIL] Raw model output for sample {i+1}:\n{full_output}\n")
                item["mixtral8x7b_scores"] = {"error": "parsing failed"}
                fout.write(json.dumps(item, ensure_ascii=False) + "\n")
            if i < 5:
                print(f"\n=== Sample {i+1} ===")
                if scores:
